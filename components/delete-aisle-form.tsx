...
import { deleteAisle } from "@/actions/merchants";
...
export default function DeleteAisleForm({
  userId,
  squadId,
  merchantId,
  aisleId,
  ...
  const t = useTranslations("DeleteAisleForm");
  ...
  const onConfirm = () => {
    startTransition(async () => {
      const res = await deleteAisle(userId, squadId, merchantId, aisleId);
      if (res?.error) {
      ...
      <BottomDrawer open={open} onOpenChange={setOpen} title={t("title")}>
        <div className="space-y-4">
          <p className="text-sm text-sand">
            {t("confirmation", { aisle: aisleName })}
          </p>
