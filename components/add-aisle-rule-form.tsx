...
import { createAisleRule } from "@/actions/merchants";
...
export default function AddAisleRuleForm({
  userId,
  squadId,
  merchantId,
  missionItems,
  ...
  const t = useTranslations("AddAisleRuleForm");
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createAisleRule(
        userId,
        squadId,
        merchantId,
        missionItemId,
      ...
      if (res?.error) {
        ...
        setMissionItemId(null);
        setMerchantAisleId(null);
        setError(null);
        setOpen(false);
        router.refresh();
      }
    ...
    <>
      ...
      <BottomDrawer open={open} onOpenChange={setOpen} title={t("title")}>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          >
            {isPending ? t("creating") : t("createRule")}
          </Button>
        </form>
      </BottomDrawer>
    </>
