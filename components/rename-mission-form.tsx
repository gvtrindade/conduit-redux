...
import { renameMission } from "@/actions/missions";
...
export default function RenameMissionForm({
  open,
  onOpenChange,
  userId,
  squadId,
  ...
  const t = useTranslations("RenameMissionForm");
  const [value, setValue] = useState(missionTitle);
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await renameMission(userId, squadId, missionId, trimmed);
      if (res?.error) {
        setError(
